--- STEAMODDED HEADER
--- MOD_NAME: Blind Collection Patch
--- MOD_ID: BlindCollectionPatch
--- MOD_AUTHOR: [Mysthaps]
--- MOD_DESCRIPTION: Patches several functions to allow adding and displaying Blinds properly
...
function Blind.set_blind(self, blind, reset, silent)
    if not reset then
        if blind and blind.atlas then
            self.children.animatedSprite.atlas = G.ANIMATION_ATLAS[blind.atlas]
        else
...
local loadref = Blind.load
function Blind.load(self, blindTable)
    if G.P_BLINDS[blindTable.config_blind].atlas then
        self.children.animatedSprite.atlas = G.ANIMATION_ATLAS[G.P_BLINDS[blindTable.config_blind].atlas]
    end
    loadref(self, blindTable)
end